...
function DetailOrder() {
    ...
    return (
        <>
            <Box>
                <Typography mt={2.5} fontSize="19px" fontWeight={300}>Chi tiết đơn hàng ID: #{order?._id}</Typography>
                {/* <Typography mt={2.5} fontSize="19px" fontWeight={300}>Chi tiết đơn hàng #{order?._id} - <span style={{ fontWeight: 500 }}>Huỷ</span></Typography> */}
                ...
                <Stack direction="row" mt={1.25} mb={2.5} className="detailOrder" justifyContent="space-between">
                    <Box className="detailOrder__boxInfo">
                        ...
                        <Box p={1.25} className="detailOrder__content">
                            <Typography style={{ color: "#000", fontWeight: 500 }}>{order?.address?.name}</Typography>
                            <Typography>
                                Địa chỉ: {order?.address?`${order?.address?.details}, ${order?.address?.ward},
                                  ${order?.address?.district},
                                  ${order?.address?.province}`:'Trống'
                                }
                            </Typography>
                    ...
                    order && <Stack direction="column"
                        ...
                        <Stack py={0.625} direction="row">
                            ...
                            <Typography className="detailOrder__summary-value detailOrder__summary-value--final">
                                {numWithCommas(order.totalPrice + order.feeShip|| 0)} ₫</Typography>
                        </Stack>
                    </Stack>
                }
